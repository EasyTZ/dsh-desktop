...
**最新版 v1.4.0** —— 设置页新增 **插件管理**（设置 → 插件 → 桌面插件，自带插件可逐个开关，重启后生效）；内核崩溃对话框新增「**安全模式启动**」，只加载插件管理面板，进去关掉出问题的插件就能恢复正常启动——以前遇到这种情况只能重装；顺手修掉一类潜在的启动失败（自带插件的内部标识可能与内核内置条目重名）。另外，Git 面板、终端面板、余额显示、在资源管理器中打开这四个插件已拆成独立开源仓库（GitHub 上打了 `dsh-plugin` 标签），其他 dsh 用户也能装到自己的环境里。
