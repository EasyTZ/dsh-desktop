...
**最新版 v1.4.3** —— Git 面板与终端面板的背板改成和左侧菜单一个底色。之前它们是浮在界面上的一块中灰色，跟应用其余部分不搭；顺带把输入框、下拉菜单、终端标签页也换成配套的颜色，深色下层次更清楚。

**v1.4.2** —— Git 面板配色修正：之前面板里有一批颜色**不跟随主题**（次级按钮、列表悬停高亮、状态标记等都是写死的灰），浅色模式下尤其难看。现在全部改用 dsh 自身的设计色，深浅两个主题下都是干净的黑白对比。
